...
describe('POST /v1/render (minify=true)', () => {
  ...
  it('error on invalid mjml', (done) => {
    request(server)
      .post('/v1/render')
      .send({
        mjml: '<mjml><mj-body><mj-section><mj-column><mj-text unknown=attribute>Hello World!</mj-text></mj-column></mj-section></mj-body></mjml>',
        errors: []
      })
      .expect('Content-Type', /json/)
      .expect({
        ...
        errors: [
          {
            line: 1,
            message: 'Attribute unknown is illegal',
            tagName: 'mj-text',
            formattedMessage: `Line 1 of ${process.cwd()} (mj-text) — Attribute unknown is illegal`
          }
        ]
      })
      .expect(200, done);
